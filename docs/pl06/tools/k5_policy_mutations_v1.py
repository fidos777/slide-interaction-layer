...
FIXTURES = [
    ...
    dict(fid="KB-25", title="several slots per day become unconditional throughput",
         target="P", gates=["MULTIPLE_SLOTS_ONLY_ON_A_CONFIRMED_AVAILABLE_DAY",
                            "SLOTS_PER_DAY_ARE_NOT_INVENTED"],
         patch=lambda: dict(POLICY=dict(P.POLICY, slots_per_day="several per day"))),
    dict(fid="KB-26", title="availability stops being conditional in the policy",
         target="P", gates=["MULTIPLE_SLOTS_ONLY_ON_A_CONFIRMED_AVAILABLE_DAY"],
         patch=lambda: dict(POLICY=dict(P.POLICY, availability="ALL_WORKING_DAYS"))),
    dict(fid="KB-27", title="what Firdaus must confirm is quietly narrowed", target="P",
         gates=["B4_NAMES_WHAT_FIRDAUS_MUST_CONFIRM"],
         patch=_dec("B4", requires_firdaus_confirmation=["the actual available dates"])),
