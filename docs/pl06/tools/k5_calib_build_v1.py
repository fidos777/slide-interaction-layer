...
# ---------------------------------------------------------- MID-MIGRATION STATE FLAG ------
# Set while the F4 migration is in flight. The handoff is the only file in the review folder
# that DESCRIBES the binaries, so when the two disagree the handoff must say so in itself —
# a note in a chat log protects nobody who pulls this branch next week. Clear this once the
# full regeneration, QA, mutation and native-render pass has run.
MID_MIGRATION = True
MID_MIGRATION_DATE = "2026-08-05"
MID_MIGRATION_COMMIT = "f082dd2"
MID_MIGRATION_STATE = "B03_F4_MIGRATION_IN_FLIGHT"
MID_MIGRATION_NOTES = [
    ...
    "Golden-master page-break map DIJANGKA GAGAL sehingga perubahan F4 disemak dan "
    "dibekukan semula dengan sebab direkodkan.",
...
def emit_handoff(qa=None, mutations=None, native=None):
    ...
    warn = ([f"> ## ⚠ {MID_MIGRATION_STATE}", ">",
             f"> **Setakat {MID_MIGRATION_DATE}, commit `{MID_MIGRATION_COMMIT}` — fail ini "
             "dan fail PPTX dalam folder yang sama adalah keluaran PERTENGAHAN MIGRASI.**", ">"]
            + [f"> - {n}" for n in MID_MIGRATION_NOTES]
            + [">",
               "> Kiraan bait, bilangan slaid, nilai QA dan status render di bawah adalah "
               "daripada larian sebelum migrasi F4 dan TIDAK sepadan dengan binari semasa.",
               ""] if MID_MIGRATION else [])
    L = warn + [f"<!-- GENERATED by {M.GENERATED_BY} — Stage {M.STAGE}. Edit the model, "
         f"not this file. -->", "",
